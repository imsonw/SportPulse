...
## Phụ lục A — Ánh xạ cho SportPulse (React Native / Expo)

Tài liệu gốc ở trên viết cho stack **Native iOS (Swift)**. SportPulse là **Expo / React Native**,
nên **giữ nguyên schema artifact và Sprint Lifecycle**, chỉ thay phần lệnh và convention như sau.

### QA Checklist — bản RN/Expo

| Bước gốc (Swift) | Bản dùng cho SportPulse |
|---|---|
| `xcodebuild build` | `npx tsc --noEmit` **và** `npx expo run:ios` build thành công |
| `swiftlint` | `npm run lint` (ESLint + eslint-config-expo) |
| `swiftformat --lint` | `npm run format:check` (Prettier `--check`) |
| Convention audit (Clean Architecture Swift) | Convention audit Feature-First (bảng dưới) |

### Convention audit — SportPulse

1. **Layering**: `app/` (route + composition) → `src/features/*` (hooks, components, api) →
   `src/lib` (queryClient, ws-client, env) → `src/stores` (Zustand). Route file **không** gọi
   fetch/axios trực tiếp.
2. **Ranh giới state**: server data (matches, leaderboard, câu hỏi) chỉ sống trong TanStack Query
   cache — **cấm** copy sang Zustand. Zustand chỉ giữ client state (auth, sound/haptic setting,
   phiên trivia đang chơi).
3. **Config tập trung**: mọi base URL / WS URL đọc từ `src/lib/env.ts` (bọc `expo-constants` +
   `app.json > extra`). Không hardcode domain trong feature.
4. **Native qua config plugin**: quyền và cấu hình native khai báo trong `app.json`. **Cấm** sửa tay
   `ios/` hay `android/` — đó là artifact sinh ra bởi prebuild.
5. **Animation trên UI thread**: dùng Reanimated 3 worklets (`useSharedValue`, `useAnimatedStyle`).
   Cấm `Animated` legacy hoặc `setInterval` cho countdown hiển thị.
6. **Native module**: chỉ expose qua `modules/audio-haptics/index.ts` có TypeScript types;
   Presentation không import trực tiếp `requireNativeModule`.
7. **Type safety**: `strict: true`, không thêm `any` ngầm hoặc `@ts-ignore` mới.

### Testing Policy
Giữ nguyên như tài liệu gốc: **unit test tắt toàn dự án**. DEV không viết test, QA không chạy test
suite. Gate của QA = typecheck + lint/format + convention audit + kiểm thủ công acceptance criteria.
